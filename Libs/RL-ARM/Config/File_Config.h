...
#pragma pop
  ...
  typedef struct
  {
    ...
    (void);
    BOOL (*ReadData)
    (U32 adr, U32 sz, U8 *buf); /* Optional, =NULL for     */
                                /* memory-mapped Parallel Flash      */
    BOOL (*ProgramPage)
    (U32 adr, U32 sz, U8 *buf);
    BOOL (*EraseSector)
    (U32 adr);
    BOOL (*EraseChip)
  ...
  typedef struct
  {
    ...
    (NAND_DRV_CFG *cfg);
    U32 (*PageRead)
    (U32 row, U8 *buf, NAND_DRV_CFG *cfg);
    U32 (*PageWrite)
    (U32 row, U8 *buf, NAND_DRV_CFG *cfg);
    U32 (*BlockErase)
    (U32 row, NAND_DRV_CFG *cfg);
  } const NAND_DRV;
  ...
  typedef struct
  {
    ...
    (U32 kbaud);
    BOOL (*Command)
    (U8 cmd, U32 arg, U32 resp_type, U32 *rp);
    BOOL (*ReadBlock)
    (U32 bl, U8 *buf, U32 cnt);
    BOOL (*WriteBlock)
    (U32 bl, U8 *buf, U32 cnt);
    BOOL (*SetDma)
    (U32 mode, U8 *buf, U32 cnt); /* Optional, =NULL for   */
