...
void send_DHCP_DISCOVER(void)
{
    ...
    // for(uint8 i=0; i<3; i++)
    // Delay_ms(800);
    socket_sendto(SOCK_DHCP, (uint8 *)pRIPMSG, sizeof(RIP_MSG), ip, DHCP_SERVER_PORT);
...
void send_DHCP_REQUEST(void)
{
    ...
    socket_sendto(SOCK_DHCP, (const uint8 *)pRIPMSG, sizeof(RIP_MSG), ip, DHCP_SERVER_PORT);
#ifdef DHCP_DEBUG
...
uint8 parseDHCPMSG(uint16 length)
{
    ...
    len = socket_recvfrom(SOCK_DHCP, (uint8 *)pRIPMSG, length, svr_addr, &svr_port);
...
uint8 check_DHCP_state(SOCKET s)
{
    ...
    else /*socket处于关闭状态，重新初始化socket*/
    {
        ...
        if (!socket_new(SOCK_DHCP, Sn_MR_UDP, DHCP_CLIENT_PORT, 0x00)) /*初始化socket和端口*/
        {
#ifdef DHCP_DEBUG
...
uint8 check_leasedIP(void)
{
    ...
    if (socket_sendto(SOCK_DHCP, (const uint8 *)"CHECK_IP_CONFLICT", 17, GET_SIP, 5000))
    {
#ifdef DHCP_DEBUG
        printf(" Conflict>\r\n");
#endif
