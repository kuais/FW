...
#define EXTERN_DNS_SERVERIP w5500cfg.dns
...
uint8 dns_query(uint8 s, uint8 *name)
{
    ...
    case SOCK_CLOSED:
		...
		if (dns_port++ == 4000)
			dns_port = 3000;
        socket_new(s, Sn_MR_UDP, dns_port, 0); /*打开W5500的socket的3000端口，并设置为UDP模式*/
        break;
    case SOCK_UDP:                                                         /*socket已打开*/
        len = dns_makequery(0, name, BUFPUB, MAX_DNS_BUF_SIZE);            /*接收DNS请求报文并存入BUFPUB*/
        socket_sendto(s, BUFPUB, len, EXTERN_DNS_SERVERIP, IPPORT_DOMAIN); /*发送DNS请求报文给DNS服务器*/
        if ((len = getSn_RX_RSR(s)) > 0)
        {
            if (len > MAX_DNS_BUF_SIZE)
                len = MAX_DNS_BUF_SIZE;
            len = socket_recvfrom(s, BUFPUB, len, ip, &dns_port); /*接收UDP传输的数据并存入BUFPUB*/
            if (parseMSG(&dhp, BUFPUB))                       /*解析DNS响应信息*/
            {
                socket_close(s);        /*关闭socket*/
        ...
        {
            os_dly_wait(1000); /*没有收到DNS服务器的UDP回复，避免太频繁，延时1s*/
 ...
 */
int dns_gethostbyname(unsigned char *url, unsigned char *remoteip)
{
    int ret = 0;
    int retryCount = 0;
    if (memcmp(w5500cfg.dns, "\x00\x00\x00\x00", 4) == 0)
    {
        printf("Invalid DNS server [%d.%d.%d.%d]\r\n", w5500cfg.dns[0], w5500cfg.dns[1], w5500cfg.dns[2], w5500cfg.dns[3]);
        return ret;
    }
	if (getSn_SR(SOCK_DNS) != SOCK_CLOSED) /*获取socket状态*/
		socket_close(SOCK_DNS);
    while (1)
    {
        if ((ret == 1) || (retryCount > DNS_RETRY))
            return ret;
        switch (dns_query(SOCK_DNS, url)) /*发出DNS请求报文和解析DNS响应报文*/
        {
        case DNS_RET_SUCCESS:                /*DNS解析域名成功*/
            ret = 1;                         /*DNS运行标志位置1*/
            retryCount = 0;                  /*DNS请求报文次数置0*/
            memcpy(remoteip, dns_get_ip, 4); /*把解析到的IP地址复制给ConfigMsg.rip*/
            printf("Get [%s]'s IP address [%d.%d.%d.%d] from [%d.%d.%d.%d]\r\n", url, remoteip[0], remoteip[1], remoteip[2], remoteip[3], w5500cfg.dns[0],
                   w5500cfg.dns[1], w5500cfg.dns[2], w5500cfg.dns[3]);
            break;
        case DNS_RET_FAIL: /*DNS解析域名失败*/
            ret = 0;       /*DNS运行标志位置0*/
            retryCount++;  /*DNS请求报文次数加1*/
            printf("Get Ip Fail %d!.\r\n", retryCount);
            break;
        default:
            break;
        }
...
//    else if (memcmp(w5500cfg.dns, "\x00\x00\x00\x00", 4)) /*判断DNS服务器的IP地址是否配置*/
//    {
...
//            printf("Get [%s]'s IP address [%d.%d.%d.%d] from %d.%d.%d.%d\r\n", domain_name, dns_get_ip[0], dns_get_ip[1], dns_get_ip[2], dns_get_ip[3],
//                   w5500cfg.dns[0], w5500cfg.dns[1], w5500cfg.dns[2], w5500cfg.dns[3]);
//            break;
